...
export class PackageInstaller {
    async install(packageId) {
        ...
        const context = new InstallerContext(process.cwd());
        ...
        try {
            ...
            for (const pkg of packages) {
                ...
                dependencyGraph.addPackage(manifest.id, manifest.dependencies ?? []);
            }
            /*
             * Validate dependency graph
             */
            const analyzer = new DependencyAnalyzer(dependencyGraph);
            analyzer.checkCircularDependencies();
            /*
             * Compute installation order
             */
            const sorter = new TopologicalSorter(dependencyGraph);
            const installationOrder = sorter.sort();
            console.log();
             ...
             */
            const scheduler = new InstallationScheduler(dependencyGraph);
            const batches = scheduler.createBatches();
            ...
            for (const batch of batches) {
                await Promise.all(batch.map(pkg => worker.install(pkg, context)));
            }
