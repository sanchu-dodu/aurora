...
export class UpdateManager {
    ...
    async check(packageId, projectPath) {
        ...
        return {
            ...
            updateAvailable: current.version !== latest,
            backup,
            plan
        };
    }
    async executeUpdate(packageId, projectPath, backupPath, currentVersion, targetVersion) {
        const context = new InstallerContext(projectPath);
        const executor = new UpdateExecutor();
        const rollback = new RollbackManager(projectPath);
        const transactions = new TransactionManager(projectPath);
        const transactionId = `update-${packageId}-${Date.now()}`;
        ...
        try {
            await executor.execute(packageId, context);
