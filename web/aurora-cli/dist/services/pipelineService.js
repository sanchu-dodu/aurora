...
export async function runProjectPipeline(framework, projectName) {
    const context = createPipelineContext(framework, projectName);
    const pipeline = new PipelineRunner();
    pipeline.addStep(new ValidateProjectStep(projectName));
    pipeline.addStep(new CreateProjectStep(context));
    pipeline.addStep(new InitializeGitStep(projectName));
    pipeline.addStep(new ConfigureAuroraStep(projectName));
    await pipeline.run();
}
